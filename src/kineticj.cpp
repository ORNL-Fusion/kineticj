...
C3Vec kj_interp1D ( const float &x, const vector<float> &xVec, const vector<C3Vec> &yVec, int &stat ) {
	...
	if(x0==x1) {
#if DEBUGLEVEL >= 2
        cout << "Stat version of kj_interp1D" << endl;
        ...
        for(int i=0;i<nPx;i++) {
            for(int j=0;j<nPy;j++) {
                for(int k=0;k<nPz;k++) {
                    ...
                    float vxL = thisvx-h;
                    float vxR = thisvx+h;
		            float fL = maxwellian(vxL,thisvy,thisvz,vTh);
		            float fR = maxwellian(vxR,thisvy,thisvz,vTh);
                    ...
                    float vyL = thisvy-h;
                    float vyR = thisvy+h;
		            fL = maxwellian(thisvx,vyL,thisvz,vTh);
		            fR = maxwellian(thisvx,vyR,thisvz,vTh);
			        df0_dv = (-fL + fR)/(2*h); 
                    ...
                    float vzL = thisvz-h;
                    float vzR = thisvz+h;
		            fL = maxwellian(thisvx,thisvy,vzL,vTh);
		            fR = maxwellian(thisvx,thisvy,vzR,vTh);
			        df0_dv = (-fL + fR)/(2*h); 
