...
float eval_vPer(CParticle& p, const C3Vec r, float *r_b0, C3Vec *b0_CYL, int n)
{
    ...
    int status = 0;
    C3Vec This_b0_CYL = kj_interp1D(r.c1, r_b0, b0_CYL, n, status);
    p.status = max(p.status, status);
...
C3Vec eval_vGC(CParticle& p, const C3Vec r, const float vPer, const float vPar,
    float *r_b0, C3Vec *b0_CYL, int n, 
    float *r_GC, C3Vec *curv_CYL, C3Vec *grad_CYL, int nGC)
{
    ...
    status = 0;
    C3Vec This_curv_CYL = kj_interp1D(r.c1, r_GC, curv_CYL, nGC, status);
    p.status = max(p.status, status);
    ...
    status = 0;
    C3Vec This_grad_CYL = kj_interp1D(r.c1, r_GC, grad_CYL, nGC, status);
    p.status = max(p.status, status);
#if DEBUG_EVAL_VGC >= 1
    if (status > 0) {
